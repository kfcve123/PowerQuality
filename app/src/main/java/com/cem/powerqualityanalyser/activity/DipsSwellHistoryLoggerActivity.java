...
public class DipsSwellHistoryLoggerActivity extends BaseActivity {
    ...
    private DipsSwellsDataBean dataCoreTable;
    ...
    protected void onResume() {
        ...
        if (dataCoreTable == null){
            showLoading();
            DBManager.getInstance().findDipsSwellsFromName(true, fileName, new DBManager.DBQueryDipsSwellsParameterListener() {
                @Override
                public void onFail() {
                    dissLoading();
                    Toast.makeText(DipsSwellHistoryLoggerActivity.this,"加载数据失败",Toast.LENGTH_SHORT).show();
                }
                ...
                public void onSuccess(List<DipsSwellsDataBean> dataMeterAllParameterList) {
                    ...
                    if (dataMeterAllParameterList!= null && dataMeterAllParameterList.size() > 0){
                        ...
                        meterList = SqlDataTool.dipsSwellsDataBean2BaseData(DipsSwellHistoryLoggerActivity.this, dataCoreTable);
                        initData();
                    }
                }
    ...
    private void refreshData(){
        if(possList!=null) {
            if (possList[popwindowsIndex] == null) {
                List<NewHistoryMeterAllObj> meterAllParamObjList = new DataToMeterTool(this).getDipsSwellsMeterAllObj(popwindowsIndex, meterList, checkedParameters);
