...
public class NewHistoryChart extends VoltAmpsMoreChartBase {
    private final int XRangeMaximum = 100;
    private final int labelCount = 10;
    private String unit = "";
    private TextView tv_time;
    private TextView tv_freq;
    public NewHistoryChart(Context context) {
        super(context);
    }

    public NewHistoryChart(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public NewHistoryChart(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public NewHistoryChart(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void initChartListener() {
        ...
        mChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry e, Highlight h) {
                String showValue = new ModelBaseData(e.getY()).getValueFl()+"";
    ...
    private List<Float> ojbToFloatValues(NewHistoryMeterAllObj meterAllObj){
        List<Float> values = new ArrayList<>();
        if(meterAllObj.getL1()!=null)
            values.add(meterAllObj.getL1().getValueFl());
        if(meterAllObj.getL2()!=null)
            values.add(meterAllObj.getL2().getValueFl());
        if(meterAllObj.getL3()!=null)
            values.add(meterAllObj.getL3().getValueFl());
        if(meterAllObj.getN()!=null)
            values.add(meterAllObj.getN().getValueFl());
        return values;
    }

    private void showTopViewValue(NewHistoryMeterAllObj meterAllObj){
        unit = meterAllObj.getUnit();
        if(meterAllObj.getL1()!=null){
            textview_l1.setText("L1    " + meterAllObj.getL1().getValue()  + "  " + meterAllObj.getL1().getValue_Unit()==null?"":meterAllObj.getL1().getValue_Unit());
        }else {
            textview_l1.setText("L1    ");
        }
        if(meterAllObj.getL2()!=null){
            textview_l2.setText("L2    " + meterAllObj.getL2().getValue()  + "  " + meterAllObj.getL2().getValue_Unit()==null?"":meterAllObj.getL2().getValue_Unit());
        }else{
            textview_l2.setText("L2    ");
        }
        if(meterAllObj.getL3()!=null){
            textview_l3.setText("L3    " + meterAllObj.getL3().getValue()  + "  " + meterAllObj.getL3().getValue_Unit()==null?"":meterAllObj.getL3().getValue_Unit());
        }else{
            textview_l3.setText("L3    ");
        }
        if(meterAllObj.getN()!=null){
            textview_l4.setText("N    "+ meterAllObj.getN().getValue()  + "  " + meterAllObj.getN().getValue_Unit()==null?"":meterAllObj.getN().getValue_Unit());
        }else{
            textview_l4.setText("N    ");
    ...
    public void showMeterAllParamObj(NewHistoryMeterAllObj meterAllObj) {
    ...
    public void showMeterAllParamObjList(List<NewHistoryMeterAllObj> meterAllObjList) {
        if (meterAllObjList != null) {
            for (int j = 0; j < meterAllObjList.size(); j++) {
                NewHistoryMeterAllObj meterAllObj = meterAllObjList.get(j);
                List<Float> values = ojbToFloatValues(meterAllObj);
                float maxLeftTrendView = Collections.max(values);
                float minLeftTrendView = Collections.min(values);
...
//                                chart.moveViewToX(lineData.getEntryCount() - XRangeMaximum);
                    ...
                    if(maxLeftTrendView>0) {
                        chart.getAxisLeft().setAxisMaximum(maxLeftTrendView * 2);
                        if(minLeftTrendView>0)
                            chart.getAxisLeft().setAxisMinimum(-minLeftTrendView * 2);
                        else if(minLeftTrendView ==0){
                            chart.getAxisLeft().setAxisMaximum(-maxLeftTrendView * 2);
                        }else{
                            chart.getAxisLeft().setAxisMinimum(minLeftTrendView * 2);
                        }

                    }else if(maxLeftTrendView == 0.0){
                        if(minLeftTrendView ==0){
                            chart.getAxisLeft().setAxisMinimum(-1);
                            chart.getAxisLeft().setAxisMaximum(1);
                        }else{
