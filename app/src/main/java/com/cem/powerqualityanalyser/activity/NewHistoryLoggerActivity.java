...
import com.cem.powerqualityanalyser.sqlBean.SqlBaseBean;
...
public class NewHistoryLoggerActivity extends BaseActivity {
 ...
 //       data.add(new BaseBottomAdapterObj(2,checkedParameters.get(0),checkedParameters.toArray(new String[checkedParameters.size()])));
    ...
    protected void onResume() {
        ...
        if (dataCoreTable == null){
            ...
            DBManager.getInstance().findFromName(true, fileName, new DBManager.DBQueryAllParameterListener() {
                ...
                public void onFail() {
                    dissLoading();
                    Toast.makeText(NewHistoryLoggerActivity.this,"加载数据失败",Toast.LENGTH_SHORT).show();
                }
                ...
                public void onSuccess(List<SqlDataBean> dataMeterAllParameterList) {
                    ...
                    if (dataMeterAllParameterList!= null && dataMeterAllParameterList.size() > 0){
                        ...
                        meterList = SqlDataTool.newSqlDataBean2BaseData(NewHistoryLoggerActivity.this, dataCoreTable);
                        initData();
                    }
                }
    ...
    private void initData(){
        ...
        updateBottomView(new BaseBottomAdapterObj(2,checkedParameters.get(0),checkedParameters.toArray(new String[checkedParameters.size()])),2);
    ...
    private void refreshData(){
        if(possList!=null) {
            if (possList[popwindowsIndex] == null) {
                List<NewHistoryMeterAllObj> meterAllParamObjList = new DataToMeterTool(this).getNewMeterAllObj(popwindowsIndex, meterList, checkedParameters);
