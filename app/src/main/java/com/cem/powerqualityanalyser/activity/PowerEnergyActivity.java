    ...
    private void setViewShow(int popIndew, int index) {
        if (popIndew == 0) {//PowerEnergyMeter 的情况
            if (index == 0) {
                ...
                updateBottomView(new BaseBottomAdapterObj(3, Res2String(R.string.Cursor), Res2String(R.string.On), Res2String(R.string.Off)), 3);
                updateBottomView(new BaseBottomAdapterObj(4, null, Res2String(R.string.Hold), Res2String(R.string.run)), 4);
...
//        log.e("========" + key.toString());
        switch (key) {
            case Up:
                ...
                } else if (Fragment_three != null && Fragment_three.isAdded()) {
                    if (currentView != null && currentView.isFocusable() && currentView instanceof RecyclerView) {
                        Fragment_three.leftUpScroll();
                    }
                } else if (null != Fragment_first && Fragment_first.isAdded()) {
                    if (cursorEnable) {
                        if (zoomSize < 3) {
                            zoomSize++;
                            AppConfig.getInstance().setMaxZoom(zoomSize);
                        }
                        Fragment_first.zoomScale(zoomSize);
...
//                        updateBottomView(new BaseBottomAdapterObj(3, Res2String(R.string.Cursor), R.mipmap.left_right, AppConfig.getInstance().getMaxZoom()), 3);
            ...
            case Right:
                if (Fragment_Second != null && Fragment_Second.isAdded()) {
                    ...
                    Fragment_three.setFocusOnRight();
                } else if (Fragment_first != null && Fragment_first.isAdded()) {
                    Fragment_first.moveCursor(1);
                }
                break;
